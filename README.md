...
This repo contains configuration files needed for running RadixDLT nodes using
nginx as a reverse proxy.

# Artifacts for standalone nginx

For node runners that do not want to use the radixdlt/radixdlt-nginx there is a
script that will generate the configuration for Archive and Fullnodes.
...
The executable components of the Babylon nginx Configuration/Code are licensed
under the [Radix Software EULA](http://www.radixdlt.com/terms/genericEULA).

The Babylon nginx Configuration/Code is released under the
[Radix License 1.0 (modified Apache 2.0)](LICENSE):
